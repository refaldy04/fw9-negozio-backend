...
exports.createProduct = async (req, res) => {
  ...
  const product = await productModel.createProductWithOption2(
    currentUser.id,
    picture,
    req.body
  );
...
exports.getAllProductsUser = async (req, res) => {
  ...
  const product = await productModel.getProductsUser(
    offset,
    limit,
    sortBy,
    typeSort,
  ...
  if (product?.length < 1) {
    ...
    pageInfo.nextPage =
      pageInfo.currentPage < pageInfo.pages ? pageInfo.currentPage + 1 : null;
...
exports.updateProduct = async (req, res) => {
  ...
  const product = await productModel.updateProductUser(
...
exports.updateProductOption = async (req, res) => {
  ...
  const productOption = await productModel.updateProductOption(
    parseInt(idOption, 10),
    picture,
    req.body
  );
