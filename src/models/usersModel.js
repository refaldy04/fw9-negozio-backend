...
exports.getAllUserCustomers = (
  ...
  ${
    ...
    ORDER BY ${
      sortBy == null ? "id DESC" : `${sortBy} ${sortType}`
    } LIMIT $1 OFFSET $2 `;
      ...
      ? `${
          searchBy === "username"
            ...
            : `${
                searchBy === "email"
                  ? "email"
                  : `${searchBy === "store_name" ? "store_name" : ""}`
              }`
        } LIKE '%${keyword}%' AND`
      : ""
  } role='seller' 
...
exports.getAllUsers2 = async (
  ...
  const users = await prisma.users.findMany({
    ...
    where: {
      ...(searchBy === "email"
        ? { email: keywordContains }
        : searchBy === "phone_number"
        ? { phone_number: keywordContains }
        : searchBy === "store_name"
        ? { store_name: keywordContains }
        : searchBy === "full_name"
        ? { full_name: keywordContains }
        : {}),
    },
    orderBy: {
